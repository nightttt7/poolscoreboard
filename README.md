...
- 玩家只需要填写名字，不需要账号密码登录
- 同时保留一个固定 `admin` 账号，并在 `/admin` 提供独立的 Admin 登录和管理后台
- 身份通过数据库保存的 Cookie 会话校验
- 一个玩家同一时间只能在一场比赛里
- 创建比赛时会优先生成随机两位数编号；两位数用满后会自动扩展到更多位
- 每场比赛最多两位玩家，任意一位退出后可由新玩家补位
- 首局默认由建房玩家开球；如果建房玩家离开，则由留下来的玩家接手首局开球方
- 双方都可以修改目标局数、每局胜负和双方犯规次数
- 每局都会显示开球方，用户可以手动更换；下一局默认自动轮换到上一局未开球的一方
- 比赛中的目标局数、胜负和犯规更新会实时同步到同一场比赛的其他页面
- 总比分只读，达到目标局数后会自动宣布胜利
- `/admin` 可以查看正在进行中的比赛，以及已经归档的历史对局详情
- 历史对局会在分出胜负、双方都离开导致关闭、或 6 小时无人操作被清理时归档
- 同一个比赛编号在重置后会开启新的历史会话，不会覆盖之前的归档
...
2. 创建新比赛，或输入比赛编号加入比赛
3. 直接在手机页面上操作计分板
4. 如需管理视图，访问 `/admin` 并使用 `ADMIN_PASSWORD` 登录

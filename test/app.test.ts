...
describe("pool scoreboard app", () => {
  ...
  it("renders the player shell without the admin password field", async () => {
    ...
    expect(html).toContain("Create Match");
    expect(html).toContain("Create a match and share the code with the other player.");
  ...
  it("requires an authenticated admin session before reading the admin dashboard", async () => {
    ...
    expect(res.status).toBe(401);
    await expect(res.json()).resolves.toEqual({ error: "Sign in as admin first" });
  });
  ...
  it("creates, joins, scores, and announces a match winner", async () => {
    ...
    expect(secondWinBody.match.winnerMessage).toContain("Alice wins the match");
